      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <AnimatedReveal direction="up" delay={50}>
          <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
            <div>
              <span
                className={`inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider mb-1 ${isDark ? 'text-[#E0B15A]' : 'text-[#B8792F]'
                  }`}
              >
        ...
        <AnimatedReveal direction="up" delay={120}>
          <div
            className={`grid grid-cols-3 py-3 sm:py-4 mb-6 sm:mb-8 border-y ${isDark ? 'border-[#4A2C16] divide-[#4A2C16]' : 'border-[#E4D4BC] divide-[#E4D4BC]'
              } divide-x`}
          >
            {/* Benefit 1 */}
            ...
            return (
              <AnimatedReveal key={pkg.id} direction="up" delay={100 + idx * 75} className="self-start h-fit w-full">
                ...
                >
                  <div>
                        ...
                        if (isSoldOut) {
                          return (
                            <div className="absolute top-1.5 right-1.5 sm:top-2.5 sm:right-2.5">
                              <span className="text-[8px] sm:text-[9.5px] font-bold px-2 py-0.5 rounded-full bg-red-600 text-white shadow-md">
                    ...
                    <div className="p-2 sm:p-4 space-y-1.5 sm:space-y-3">
                      ...
                      <button
                        ...
                        className={`w-full text-[9.5px] sm:text-xs font-semibold flex items-center justify-between py-1.5 px-2 rounded-lg transition-all duration-200 border cursor-pointer ${isExpanded
                  ...
                  {/* Price & Action */}
                  <div
                    className={`border-t flex flex-col justify-between ${isDark ? 'bg-[#24170D] border-[#4A2C16]' : 'bg-[#FAF7F0] border-[#E4D4BC]'
                      }`}
                  >
                      ...
                      if (isSoldOut) {
                        return (
                          <button
                            type="button"
                            disabled
                            className="w-full py-2.5 sm:py-3.5 rounded-t-none rounded-b-2xl sm:rounded-b-3xl bg-stone-700/60 text-stone-300 font-bold text-[10px] sm:text-xs cursor-not-allowed opacity-80 flex items-center justify-center gap-1.5"
                          >
